...
LangGraph 风格的图编排（简化实现，不强依赖 LangGraph 库）：
  Monitor → RCA → Heal → Change → Report → End
     ↓         ↓       ↓
   (skip)   (skip)  (rejected → retry/escalate)

为什么用状态机而不是线性 Pipeline：
Pipeline 无法处理条件分支、重试、跳过、回退。状态机支持：
   - 条件路由（RCA 置信度低 → 重新分析）
...
class Orchestrator:
    ...
    def _build_default_workflow(self) -> None:
        """构建默认工作流：Monitor → RCA → Heal → Change → Report"""
        monitor = MonitorAgent(self.event_bus)
        rca = RCAAgent(self.event_bus, knowledge_graph=self.knowledge_graph)
        heal = HealAgent(self.event_bus, dry_run=True)
        change = ChangeAgent(self.event_bus)
        report = ReportAgent(self.event_bus)
        ...
        self._nodes = [
            ...
            WorkflowNode(
                ...
                agent=report,
                condition=lambda s: (
                    report.is_available() and s.alert_event is not None
                ),
            ),
        ]
...
async def run_demo():
    ...
    if state.report_event:
        print(f"\n  [复盘报告]")
        print(f"    模型:     {state.report_event.model}")
        print(f"    {'-' * 4}")
        print(state.report_event.content)
