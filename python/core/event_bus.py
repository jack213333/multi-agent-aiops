...
class InMemoryEventBus(EventBus):
    ...
    2. 单元测试不依赖外部服务
    3. 依赖倒置原则：Agent 只面向 EventBus 抽象编程，实现可替换
    """
...
class KafkaEventBus(EventBus):
    ...
    def check_connection(self, timeout: float = 5.0) -> None:
        """探测 broker 连通性（启动时 fail-fast 用）

        confluent-kafka 是惰性连接：Producer/Consumer 构造时并不建立连接，
        因此需要主动探测，否则 broker 没启动时应用也能"正常"启动，
        直到第一条消息才报错。list_topics 会阻塞等待元数据，
        连不上时抛 KafkaException。
        """
        from confluent_kafka import Producer

        probe = Producer({"bootstrap.servers": self._bootstrap_servers})
        try:
            probe.list_topics(timeout=timeout)
        finally:
            probe.flush(timeout=1)
