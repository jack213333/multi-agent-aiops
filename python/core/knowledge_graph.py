...
class Neo4jKnowledgeGraph:
    ...
    def find_root_causes(self, service: str, max_depth: int = 5) -> list[dict]:
        ...
        注意：Neo4j 的变长路径模式 (:DEPENDS_ON*1..n) 不支持参数化上界，
        必须内联为字面量（max_depth 来自内部配置，非用户输入）。
        """
        depth = int(max_depth)
        with self._driver.session() as session:
            result = session.run(
                f"""
                MATCH path = (s:Service {{name: $service}})-[:DEPENDS_ON*1..{depth}]->(root)
    ...
    # --- 与 InMemoryKnowledgeGraph 对齐的查询接口（供 API 与 RCA Agent 使用）---

    def seed_demo_data(self) -> None:
        """灌入演示拓扑数据（与内存版同一份数据定义，幂等可重复执行）"""
        for name, props in DEMO_SERVICES.items():
            self.add_service(name, props)

        for source, target, _ in DEMO_DEPENDENCIES:
            self.add_dependency(source, target)

        for svc, node, _ in DEMO_DEPLOYMENTS:
            self.add_runs_on(svc, node)

    def add_runs_on(self, service: str, node: str) -> None:
        """写入部署关系:Service -[:RUNS_ON]-> Node"""
        with self._driver.session() as session:
            session.run(
                """
    ...
    def get_dependencies(self, service: str) -> list[str]:
        """某服务的所有直接依赖（一跳 DEPENDS_ON）"""
        with self._driver.session() as session:
            result = session.run(
                """
                MATCH (s:Service {name: $service})-[:DEPENDS_ON]->(t)
    ...
    def get_dependents(self, service: str) -> list[str]:
        ...
        with self._driver.session() as session:
            result = session.run(
                """
                MATCH (up)-[:DEPENDS_ON]->(s:Service {name: $service})
    ...
    def compute_impact_score(self, service: str) -> float:
        """影响力得分 = 直接上游数量 / 服务总数（与内存版同口径）"""
        with self._driver.session() as session:
            result = session.run(
                """
                MATCH (s:Service)
                WITH count(s) AS total
                MATCH (target:Service {name: $service})
                OPTIONAL MATCH (up)-[:DEPENDS_ON]->(target)
                RETURN total, count(up) AS dependents
                """,
                service=service,
            )
            ...
            if not record or record["total"] == 0:
                return 0.0
            return record["dependents"] / record["total"]

    def bfs_trace(self, start: str, rel_type: str = "DEPENDS_ON", max_depth: int = 5) -> list[list[str]]:
        """BFS 遍历：返回从 start 出发的所有路径（多跳 Cypher 查询）

        可变长路径模式 (:DEPENDS_ON*1..n) 是图数据库
        相比关系型递归 CTE 的核心优势；上界需内联字面量（见 find_root_causes）。
        """
        depth = int(max_depth)
        with self._driver.session() as session:
            result = session.run(
                f"""
                MATCH path = (s:Service {{name: $start}})-[:DEPENDS_ON*1..{depth}]->(leaf)
    ...
    def reverse_bfs_trace(self, start: str, rel_type: str = "DEPENDS_ON", max_depth: int = 5) -> list[str]:
        """反向 BFS：沿依赖链找到所有根因候选（叶子节点）

        与 find_root_causes 的 Cypher 相同，返回结构对齐内存版接口。
        """
        root_causes = self.find_root_causes(start, max_depth=max_depth)
        seen: set[str] = set()
        result: list[str] = []
    ...
    def get_topology_summary(self) -> dict[str, Any]:
        ...
        with self._driver.session() as session:
            ...
            node_types = {record["type"]: record["cnt"] for record in type_stats}
            names = session.run("MATCH (n) RETURN n.name AS name ORDER BY name")
            return {
                "total_nodes": node_stats["nodes"] if node_stats else 0,
                "total_edges": edge_stats["edges"] if edge_stats else 0,
                "node_types": node_types,
                "nodes": [record["name"] for record in names],
            }
...
def create_neo4j_knowledge_graph(
    ...
    """创建 Neo4j 知识图谱并灌入演示拓扑数据（与内存版同一份数据）

    需要 Neo4j 已启动；连接失败会抛异常，由调用方决定降级或报错。
    """
...
def _seed_demo_data(kg: InMemoryKnowledgeGraph) -> None:
    """向知识图谱写入演示拓扑（内存版与 Neo4j 版共用同一份数据定义）"""
    for name, props in DEMO_SERVICES.items():
        kg.add_node(name, node_type=props["type"], properties=props)

    for source, target, rel_type in DEMO_DEPENDENCIES:
        kg.add_relationship(source, target, rel_type)

    for svc, node, rel_type in DEMO_DEPLOYMENTS:
        kg.add_node(node, "host")
        kg.add_relationship(svc, node, rel_type)


# --- 演示拓扑数据（内存版与 Neo4j 版共享）---
